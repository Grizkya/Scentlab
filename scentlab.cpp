...
// Mencetak teks agar otomatis berada di tengah boks luar (75 karakter)
void sentences(string teks, string warna = RESET) {
    int isiLayar  = 73; // 75 - 2 border pipa
...
// Mencetak teks rata kiri dengan indentasi tertentu, ujung kanan otomatis pas di kolom 75
void sentencesLeft(string teks, int indentasi = 2, string warna = RESET) {
    int isiLayar = 73; // 75 - 2 border pipa

    cout << "|";
    garis2(indentasi, ' '); // Indentasi awal (misal spasi di dalam boks)
    cout << warna << teks << RESET;

    // Hitung sisa ruang kosong yang harus diisi spasi sampai ke border kanan
...
// Mencetak baris kosong ber-border pipa samping
...
void cetakHeaderKonten(const string &judul) {
    CLS;
    garis(75, '=');
    sentences("SCENTLAB v2.6 - EXCLUSIVE PERFUME LAYERING & MANAGEMENT SYSTEM", GREEN);
    garis(75, '=');
    sentences(">>> " + judul + " <<<", YELLOW);
    garis(75, '-');
}

// Menggambar satu tombol fisik ber-border kotak di tengah tata letak 75-kolom
void borderPilihan(string teks, string warna = RESET) {
    int isiLayar  = 43; // Lebar boks tombol internal
    int sisaSpasi = isiLayar - (int)teks.length();
    int kiri      = sisaSpasi / 2;
    int kanan     = sisaSpasi - kiri;
...
int pilihMenu(const string opsi[], int jumlahOpsi, const string &judulHeader = "MENU", bool tampilSplash = false, bool merahkanTerakhir = true) {
    ...
    while (true) {
        ...
        garis(75, '=');
        sentences("SCENTLAB v2.6 - EXCLUSIVE PERFUME LAYERING MANAGEMENT", GREEN);
...
void tampilGudang(BibitParfum gudang[], int jml_gudang, bool tampilHabis = false) {
    ...
    if (!adaBahan) sentences("(Tidak ada bahan yang tersedia)");
    cout << "|"; garis2(73, '-'); cout << "|\n";
}
...
// SUBMENU Katalog Gudang & Restok Bahan
...
// SUBMENU Buka Meja Racik (Buat Parfum Baru)
...
// SUBMENU Keluar dari Program
void keluardariMenu(bool &keluar) {
    if (pilihYaTidak("Yakin ingin menutup software SCENTLAB?")) {
        CLS;
        garis(75, '=');
        sentences("SCENTLAB v2.6 - BY AMEL, FAUZAN, APRI, AKIFA, RASYAD, RIMA, DAN AL", GREEN);
